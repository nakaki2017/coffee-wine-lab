...
8. 点击详情主图可全屏查看；多图时使用左右按钮或键盘方向键切换，按 `Escape` 关闭。
9. 需要修改时进入详情点击编辑；编辑预览会完整显示图片，便于设置封面、排序或删除。
10. 需要删除时在详情操作；存在活跃批次时系统会阻止删除。
...
- 列表中的竖图会完整显示，横图可能为了保持卡片整齐而居中裁切；进入详情或全屏后可查看完整图片。

![豆种列表/空状态](../images/manual/03-bean-profile.png)
![豆种详情和完整图片](../images/manual/14-bean-detail-image.png)
...
3. 点击配方卡片查看图片、原料、参数和制作步骤；点击详情主图可全屏查看，多图可通过按钮或键盘方向键切换。
...
- 配方列表中的竖图会完整显示，横图可能居中裁切；详情、编辑预览和全屏查看始终显示完整图片。
...
8. 点击当天详情中的大图可全屏查看，按 `Escape`、关闭按钮或点击遮罩退出。
9. 保存、替换或删除当天记录。
...
| `14-bean-detail-image.png` | 豆种详情和完整显示图片 |
| `15-image-lightbox.png` | 全屏图片查看 |
